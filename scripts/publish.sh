...
if git diff --cached --quiet; then
  notify_discord "ℹ️ Massachusetts Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: MA daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
...
notify_discord "✅ Massachusetts Pay Hub updated [$TODAY]
