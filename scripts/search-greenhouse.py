#!/usr/bin/env python3
"""
ma-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Massachusetts edition.
...
Run: python3 ~/ma-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/ma-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/ma-pay-hub/scripts/.greenhouse.lock")
...
SEED_SLUGS = load_greenhouse_seeds('ma')
...
def is_ma_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_ma_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== MA Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new MA jobs written to {OUTPUT_FILE} ===")
