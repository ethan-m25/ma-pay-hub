#!/usr/bin/env python3
"""
ma-pay-hub/scripts/search-workday.py
Workday API scraper — Massachusetts edition.

MA Pay Transparency Act (effective Oct 29, 2025)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Massachusetts employers
  2. CXS JSON API   — paginate all jobs per employer, filter Massachusetts (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/ma-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/ma-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/ma-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Massachusetts"
...
SEED_TENANTS = [
    ...
    ("libertymutual.wd1.myworkdayjobs.com", "libertymutual", "LMGCareers", "Liberty Mutual"),
    ("fidelity.wd1.myworkdayjobs.com", "fidelity", "Fidelity", "Fidelity Investments"),
    ("biogen.wd1.myworkdayjobs.com", "biogen", "Biogen_External_Site", "Biogen"),
    ("wayfair.wd1.myworkdayjobs.com", "wayfair", "Wayfair", "Wayfair"),
    ("bostonscientific.wd5.myworkdayjobs.com", "bostonscientific", "External", "Boston Scientific"),
    ("rtx.wd5.myworkdayjobs.com", "rtx", "RTX", "RTX (Raytheon Technologies)"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Massachusetts" salary 2026',
    'site:myworkdayjobs.com "Boston" salary "$" 2026',
    'site:myworkdayjobs.com "Cambridge" OR "Waltham" OR "Lexington" salary 2026',
    'site:myworkdayjobs.com Massachusetts biotech OR pharma OR finance salary 2026',
    'site:myworkdayjobs.com "Woburn" OR "Burlington" OR "Bedford" MA salary 2026',
]
...
def is_massachusetts(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Massachusetts.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_massachusetts(p.get("locationsText", ""), p.get("externalPath", "")):
                    ma_jobs.append(p)
            offset += limit
        ...
        log(f"  MA jobs: {len(ma_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(ma_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_massachusetts(location, external_path):
            continue
