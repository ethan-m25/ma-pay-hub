#!/usr/bin/env bash
# ma-pay-hub/scripts/nightly-pipeline.sh
# Nightly pipeline for Massachusetts Pay Hub.
...
SCRIPTS_DIR="$HOME/ma-pay-hub/scripts"
...
log "=== MA nightly pipeline started ==="
...
# === Phase 3 discovery — hub: ma (added 2026-05-27) ===
log "--- Phase 3 discovery: hub_search_jobs ---"
python3 "$HOME/shared-scripts/hub_search_jobs.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
log "hub_search_jobs done (exit $?)"
log "--- Phase 3 discovery: hub_deep_search ---"
python3 "$HOME/shared-scripts/hub_deep_search.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
log "hub_deep_search done (exit $?)"
log "--- Phase 3 discovery: hub_search_kpmg ---"
python3 "$HOME/shared-scripts/hub_search_kpmg.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
log "hub_search_kpmg done (exit $?)"
log "--- Phase 3 discovery: hub_search_sap ---"
python3 "$HOME/shared-scripts/hub_search_sap.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
log "hub_search_sap done (exit $?)"
log "--- Phase 3 discovery: hub_search_scout ---"
python3 "$HOME/shared-scripts/hub_search_scout.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
log "hub_search_scout done (exit $?)"

# === Phase 5 classify — hub: ma (added 2026-05-28) ===
log "--- Phase 5 classify_step ---"
python3 "$HOME/shared-scripts/region_classifier/classify_step.py" \
    --hub "ma" --no-llm >> "$LOG_FILE" 2>&1 || true
log "classify_step done (exit $?)"
# === Phase 5.5 publish gate — hub: ma (added 2026-05-28) ===
...
GATE_RAW="$HOME/.openclaw/shared/ma-jobs-raw-$(date +%Y-%m-%d).txt"
if [[ -f "$GATE_RAW" ]]; then
...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only --hub "ma" >> "$LOG_FILE" 2>&1
log "healthcheck done (exit $?)"

log "--- archive-jobs ---"
python3 "$HOME/shared-scripts/hub_archive_jobs.py" --hub "ma" --limit 100 >> "$LOG_FILE" 2>&1
log "archive done (exit $?)"
log "--- skill-extract ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_skill_extract.py" --hub "ma" --limit 200 >> "$LOG_FILE" 2>&1
log "skill-extract done (exit $?)"
log "--- classify-new-jobs ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_classify_new_jobs.py" --hub "ma" >> "$LOG_FILE" 2>&1 || true
...
log "=== MA nightly pipeline complete ==="
...
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only \
  --hub ma --pipeline-exit-code "$PIPELINE_RC" || true
exit "$PIPELINE_RC"
