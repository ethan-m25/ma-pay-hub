#!/usr/bin/env python3
"""
ma-pay-hub/scripts/_common.py
Shared utilities for all Massachusetts Pay Hub search scripts.
Massachusetts Pay Transparency Act (M.G.L. c.149, §205A) — effective Oct 29, 2025 — employers with 25+ employees must include pay range
"""
...
DATA_FILE   = os.path.expanduser("~/ma-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "MA"
...
REGION_TERMS = ['massachusetts', 'boston', 'cambridge', 'somerville', 'brookline', 'newton', 'waltham', 'woburn', 'lexington, ma', 'concord, ma', 'marlborough', 'worcester', 'springfield, ma', 'lowell', 'haverhill', 'north andover', 'andover, ma', 'burlington, ma', 'framingham', 'natick', 'needham', 'canton, ma', 'quincy, ma', 'watertown, ma', 'malden', ', ma', 'ma,', 'back bay', 'kendall square', 'seaport']

_NON_REGION_LOC_TERMS = ['toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'washington, dc', 'washington dc', 'district of columbia', 'denver, co', 'boulder, co', 'seattle, wa', 'washington state', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'minneapolis, mn', 'newark, nj', 'jersey city, nj', 'baltimore, md', 'burlington, vt']
...
EXTRACT_PROMPT = """\
Extract ONE Massachusetts job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Massachusetts job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Massachusetts location","source_url":"{url}","posted":"YYYY-MM-DD"}
